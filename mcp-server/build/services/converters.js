...
function convertOsisRefToIndex(osisRef) {
    if (!osisRef || typeof osisRef !== 'string')
        return undefined;
...
function convertOsisChapterToLastVerseInChapterOsisRef(osisChapter) {
    ...
    if (!versesFound) {
        logger.error(`Unrecognised chapter: ${osisChapter}`);
        return [];
    }
...
export { convertUnityLookupToOsisChapter, convertOsisRefToBookCode, convertOsisRefToIndex, convertIndexToOsisRef, convertOsisBook2OsisChapters, convertOsisRefToOsisBook, convertOsisRefToBookName, convertOsisChapterToOsisRefs, convertOsisRangeToOsisRefs, convertOsisChapterToLastVerseInChapterOsisRef, expandOsisRef };
