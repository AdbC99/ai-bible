 ...
 */
const addGetBibleVerses = (server: McpServer) => {
    return server.tool(
        "get-bible-verses",
        "Get one or more Bible verse given a list of verse references or whole book names. ALWAYS set language to 'english' unless user explicitly requests Hebrew, Greek, original or transliteration. ALWAYS present the found results to the user. Never say 'no results found' unless the tool explicitly returns empty data. Use the returned information to answer the user's question.",
        {
            /**
            ...
            language: z
                ...
                .default("english")
                .describe(
                    'REQUIRED: Default to "english" unless user specifically asks for hebrew, greek, transliteration or original.'
                )
        },
 ...
 * @returns {McpServer} The updated server with the new tool added.
 */
const addGetBibleVersesOriginalOnly = (server: McpServer) => {
    return server.tool(
        "get-bible-verses-original-langauge",
        "Get one or more Bible verse given a list of verse references or whole book names. The language is original for this tool. ALWAYS present the found results to the user. Never say 'no results found' unless the tool explicitly returns empty data. Use the returned information to answer the user's question.",
        {
            /**
             * A bible verse reference to search for (e.g. "John.3.16-17" or "John"). Results from this search should be presented to the user in the main response
             *
